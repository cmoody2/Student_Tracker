...
def load_gui(self):
    ...
    self.scrollbar1.grid(row=1, column=9, rowspan=8, columnspan=1, padx=(0,0), pady=(0,0), sticky=N+E+S)
    self.lstList1.grid(row=1, column=2, rowspan=8, columnspan=7, padx=(0,0), pady=(0,0), sticky=N+E+S+W)

    self.btn_add = tk.Button(self.master, width=12, height=2, text='Add', command=lambda: student_tracker_func.addToList(self))
    self.btn_add.grid(row=9, column=0, padx=(30,0), pady=(45,10),sticky=W)
    ...
    self.btn_close.grid(row=9, column=4, columnspan=1, padx=(25,0), pady=(45,10), sticky=E)
