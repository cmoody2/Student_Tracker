...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tbl_student_tracker (col_fname, col_lname, col_fullname, col_phone, col_email, col_course) VALUES (?,?,?,?,?,?)""", ('John','Doe','John Doe','111-111-1111','jdoe@email.com','ART 102'))
            conn.commit()
    conn.close()
...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0): # Enforce the user to provide both names
        ...
        with conn:
            ...
            cursor.execute("""SELECT COUNT(col_fullname) FROM tbl_student_tracker WHERE col_fullname = '{}'""".format(var_fullname)) #,(var_fullname))
...
def onDelete(self):
    var_select = self.lstList1.get(self.lstList1.curselection())  # Listboxe's selected value
    conn = sqlite3.connect('db_student_tracker.db')
    with conn:
        cur = conn.cursor()
...
def onUpdate(self):
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0) and (len(var_course) > 0):  #ensure that there is data present
        ...
        with conn:
            ...
            cur.execute("""SELECT COUNT(col_phone) FROM tbl_student_tracker WHERE col_phone = '{}'""".format(var_phone))
            count = cur.fetchone()[0]
            print(count)
            cur.execute("""SELECT COUNT(col_email) FROM tbl_student_tracker WHERE col_email = '{}'""".format(var_email))
            count2 = cur.fetchone()[0]
            print(count2)
            cur.execute("""SELECT COUNT(col_course) FROM tbl_student_tracker WHERE col_course = '{}'""".format(var_course))
            count2 = cur.fetchone()[0]
            print(count3)
            if count == 0 or count2 == 0 or count3 == 0:  # If proposed changes are not already in the database, thenm proceed
                ...
                if response:
                    ...
                    with conn:
                        cursor = conn.cursor()
                        cursor.execute("""UPDATE tbl_student_tracker SET col_phone = '{0}', col_email = '{1}', col_course = '{2}' WHERE col_fullname = '{3}'""".format(var_phone,var_email,var_course,var_value))
                        onClear(self)
                        conn.commit()
                else:
        ...
        message.showerror("Missing information","Please select a name from the left. \nThen edit the phone or email information.")
    onClear(self)
